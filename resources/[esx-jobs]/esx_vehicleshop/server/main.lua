...
AddEventHandler('esx_vehicleshop:addToList', function(target, model, plate)
	...
	discordMSG(source, target, model, plate)
...
discordMSG = function(source, customer, model, plate)
    ...
    local embeds = {
        {
            ["title"]= "Ett fordon såldes av " .. sellerName["firstname"] .. " " .. sellerName["lastname"],
            ["type"]="rich",
            ["color"] = 56108,
            ["description"] = "**Kund:** " .. customerName["firstname"] .. " " .. customerName["lastname"] .. "\n**Modell:** " .. model .. "\n**Registreringsnummer:** " .. plate .. "\n**Datum:** " .. date
        }
    }
	PerformHttpRequest('https://canary.discordapp.com/api/webhooks/630416003624337428/WoQLLUGAtakAe4rsrOJGRn0ugmLTWXeBtORkmSPo10muYO_QPecqP3UbcR1derCMZ-K5', function(err, text, headers) end, 'POST', json.encode({username = "Bokföring", embeds = embeds}), { ['Content-Type'] = 'application/json' })
end
...
RegisterServerEvent('bilfirma:getStockItem')
AddEventHandler('bilfirma:getStockItem', function(itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
				...
				TriggerClientEvent("esx_vehicleshop:updateStorage", -1)
...
RegisterServerEvent('bilfirma:putStockItems')
AddEventHandler('bilfirma:putStockItems', function(itemName, count)
	local xPlayer = ESX.GetPlayerFromId(source)
	local sourceItem = xPlayer.getInventoryItem(itemName)
...
ESX.RegisterServerCallback('bilfirma:getStockItems', function(source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
...
ESX.RegisterServerCallback('bilfirma:getPlayerInventory', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('bilfirma:getArmoryWeapons', function(source, cb)
	TriggerEvent('esx_datastore:getSharedDataStore', 'society_cardealer', function(store)
		local weapons = store.get('weapons')
...
ESX.RegisterServerCallback('bilfirma:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('bilfirma:removeArmoryWeapon', function(source, cb, weaponName)
	local xPlayer = ESX.GetPlayerFromId(source)
