...
AddEventHandler('esx_windowcleaner:GiveItem', function()
	...
	if Quantity >= 20 then
		...
		TriggerClientEvent('esx:showNotification', _source, 'Du har tvättat ~g~x' .. amount .. ' fönster')
	end
	...
	SetTimeout(Config.Zones.Vente.ItemTime, function()
		...
		if PlayersVente[source] == true then
			...
			if Quantity < Config.Zones.Vente.ItemRemove then
				TriggerClientEvent('esx:showNotification', _source, '~r~Du behöver mer '..Config.Zones.Vente.ItemRequires_name..' för att lämna in.')
				...
				TriggerClientEvent('esx:showNotification', _source, 'Du fick ~g~Kr' .. Config.Zones.Vente.ItemPrice)
				Vente(_source)
			end
...
AddEventHandler('esx_windowcleaner:startVente', function()
	...
	if PlayersVente[_source] == false then
		TriggerClientEvent('esx:showNotification', _source, '~r~Avsluta och åk tillbaka !')
		PlayersVente[_source] = false
	else
		PlayersVente[_source] = true
		TriggerClientEvent('esx:showNotification', _source, '~g~Kontrollerar ~w~dina kvitton...')
		Vente(_source)
	end
end)
