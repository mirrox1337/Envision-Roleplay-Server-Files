...
local NPCLastCancel           = GetGameTimer() - 15 * 60000
        ...
        if Config.EnableSocietyOwnedVehicles then
            ...
            local elements = {
              {label = _U('flat_bed'), value = 'flatbed'},
			        {label = _U('mek_bil'), value = 'veln'},
              {label = _U('nissan_pickup'), value = 'nissantitan17'}
            }
            ...
            if targetVehicle ~= 0 then
              if not IsPedInAnyVehicle(playerped, true) then
                if vehicle ~= targetVehicle then
                  AttachEntityToEntity(targetVehicle, vehicle, 20, -0.0, -5.60, 0.95, 0.0, 0.0, 0.0, false, false, false, false, 20, true)
    ...
    while true do
        ...
        if IsControlJustReleased(0, Keys['DELETE']) and PlayerData.job ~= nil and PlayerData.job.name == 'mecano' then
          ...
          if NPCOnJob then
            ...
            if GetGameTimer() - NPCLastCancel > 15 * 60000 then
              StopNPCJob(true)
