...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if
			...
			data.current.value == 'police_wear_short' or
			data.current.value == 'police2_wear' or
			data.current.value == 'police2_wear_short' or
			data.current.value == 'sergeant_wear' or
			data.current.value == 'sergeant_wear_short' or
			data.current.value == 'sergeant2_wear' or
			data.current.value == 'sergeant2_wear_short' or
			data.current.value == 'lieutenant_wear' or
			data.current.value == 'lieutenant_wear_short' or
			data.current.value == 'commandant_wear' or
			data.current.value == 'commandant_wear_short' or
			data.current.value == 'mc_wear' or
			data.current.value == 'insats_wear'
		then
		...
		if data.current.value == 'citizen_interaction' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					--elseif action == 'pistol_krut' then
						--TriggerEvent('esx_guntest:checkGun', source)
...
function OpenBodySearchMenu(player)
	...
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(data)
	TaskPlayAnim(GetPlayerPed(-1),"anim@gangops@morgue@table@","player_search", 1.0, -1.0, 10000, 1, 1, false, false, false)
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('esx_policejob:getFineList', function(fines)
		...
		for k,fine in ipairs(fines) do
			table.insert(elements, {
				label     = ('%s <span style="color: #771282;">%s</span>'):format(fine.label, _U('armory_item', ESX.Math.GroupDigits(fine.amount))),
				value     = fine.id,
				amount    = fine.amount,
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_policejob:getVehicleFromPlate', function(owner, found)
				if found then
					...
					--exports['mythic_notify']:SendAlert('inform', _U('search_database_found', owner))
				else
					ESX.ShowNotification(_U('search_database_error_not_found'))
					--exports['mythic_notify']:SendAlert('inform', _U('search_database_error_not_found'))
				end
			end, data.value)
			menu.close()
		end
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			exports['mythic_notify']:SendAlert('inform', _U('licence_you_revoked', data.current.label, targetName))
...
function OpenBuyWeaponsMenu()
	...
	for k,v in ipairs(Config.AuthorizedWeapons[PlayerData.job.grade_name]) do
			...
			for i=1, #v.components do
				if v.components[i] then
					...
					if hasComponent then
						label = ('%s: <span style="color: #771282;">%s</span>'):format(component.label, _U('armory_owned'))
					else
						if v.components[i] > 0 then
							label = ('%s: <span style="color: #771282;">%s</span>'):format(component.label, _U('armory_item', ESX.Math.GroupDigits(v.components[i])))
						else
							label = ('%s: <span style="color: #771282;">%s</span>'):format(component.label, _U('armory_free'))
						end
					end
		...
		if hasWeapon and v.components then
			label = ('%s: <span style="color: #771282;">></span>'):format(weapon.label)
		elseif hasWeapon and not v.components then
			label = ('%s: <span style="color: #771282;">%s</span>'):format(weapon.label, _U('armory_owned'))
		else
			if v.price > 0 then
				label = ('%s: <span style="color: #771282;">%s</span>'):format(weapon.label, _U('armory_item', ESX.Math.GroupDigits(v.price)))
			else
				label = ('%s: <span style="color: #771282;">%s</span>'):format(weapon.label, _U('armory_free'))
			end
		end
...
function openJailMenu(playerid)
	...
	ESX.UI.Menu.Open(
	  ...
	  function(data3, menu)
        ...
        exports['mythic_notify']:SendAlert('inform', "Ange antalet timmar du vill sätta personen i fängelse (IRL  tid).")
			  blockinput = true
...
Citizen.CreateThread(function()
	while true do
		...
		if isHandcuffed then
			...
			--DisableControlAction(0, Keys['F2'], true) -- Animations
			--DisableControlAction(0, Keys['F3'], true) -- Inventory
...
Citizen.CreateThread(function()
	AddTextEntry("0x79FBB0C5", "Volvo XC70")
	AddTextEntry("0x9F05F101", "Volkswagen Passat")
	AddTextEntry("0x1B38E955", "Volkswagen Transporter")
	AddTextEntry("0x9BAA707C", "Volvo V90 Målad")
	AddTextEntry("0x8A63C7B9", "Volvo V90 Civil")
	AddTextEntry("0xB822A1AA", "Insatsfordon")
end)
