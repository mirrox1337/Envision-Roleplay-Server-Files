...
AddEventHandler('esx_ambulancejob:giveItem', function(itemName, amount)
	...
	if xPlayer.job.name ~= 'ambulance' then
		...
		return
	elseif (itemName ~= 'medkit' and itemName ~= 'bandage' and itemName ~= 'firstaid' and itemName ~= 'gauze' and itemName ~= 'patch' and itemName ~= 'adrenaline' and itemName ~= 'morphine' and itemName ~= 'vicodin' and itemName ~= 'hydrocodone') then
		print(('esx_ambulancejob: %s attempted to spawn in an item!'):format(xPlayer.identifier))
		return
	end
