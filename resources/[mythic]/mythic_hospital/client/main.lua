ESX = nil
--local hospitalCheckin = { x = 312.52, y = -592.83, z = 43.28, h = 180.4409942627 }
...
AddEventHandler('mythic_hospital:client:RPSendToBed', function(id, data)
    ...
    Citizen.CreateThread(function()
        while bedOccupyingData ~= nil do
            Citizen.Wait(1)
            PrintHelpText('Tryck ~INPUT_VEH_DUCK~ för att kliva upp')
...
AddEventHandler('mythic_hospital:client:SendToBed', function(id, data)
    ...
    Citizen.CreateThread(function ()
        ...
        exports['mythic_notify']:DoHudText('inform', 'Du blir behandlad')
...
AddEventHandler('mythic_hospital:client:FinishServices', function()
    ...
    exports['mythic_notify']:DoHudText('inform', 'Du har blivit behandlad och fakturerad')
    LeaveBed()
end)
        ...
        if distance < 8 then
            ...
            if not IsPedInAnyVehicle(PlayerPedId(), true) then
                if distance < 1.2 then
                    PrintHelpText('Tryck ~INPUT_CONTEXT~ ~s~för att checka in')
                    --ESX.TriggerServerCallback('mythic_hospital:anyamb', function(anyamb)
                    if IsControlJustReleased(0, 54) then
                        --if anyamb == 0 then
                            ...
                            if (GetEntityHealth(PlayerPedId()) < 200) or (IsInjuredOrBleeding()) then
                                ...
                                exports['mythic_notify']:DoHudText('inform', 'Sjukvårdare är i tjänst, tillkalla personal för vård')
                            end
                            end)
                        	-- else
                            -- 	exports['mythic_notify']:DoHudText('error', 'Du är inte i behov av vård')
                        	-- end
                    		-- end)
                    end
                end
...
RegisterCommand("bed", function(source, args, raw)
  TriggerEvent('mythic_hospital:client:RPCheckPos', source)
end)]]