(function(){
	...
	window.onData = function(data){
		...
		if(data.acceptedCall === true) {
			Phone.apps['contact-action-call'].startCall(data.channel, data.target, true);
		}
		...
		if(data.callHistory === true){
			if(data.missedCall === true){
				Phone.calls.push({
				["number"]: data.caller,
				["self"]: false,
				["date"]: data.date,
