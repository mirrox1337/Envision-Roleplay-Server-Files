(function(){
	...
	app.updateTweets = function () {
		...
		for (let i = 0; i < Phone.tweets.length; i++) {
			if (!Phone.tweets[i].read) {
				unread++;
			}
		}
		
		if (unread == 0) {
			$('#app-home .menu-icon[data-app="twitter"] .counter').hide();
		} else {
			$('#app-home .menu-icon[data-app="twitter"] .counter').text(unread).show();
		}
	}
