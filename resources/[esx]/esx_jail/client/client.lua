...
function JailLogin()
	...
	ESX.ShowNotification("Senast du gick och la dig så var du i fängelse, Nu är du tillbaka för att tjäna av resten av tiden!")
...
function UnJail()
	...
	ESX.ShowNotification("Du är frisläppt, var lugn på utsidan! Lycka till!")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Du har " .. jailTime .. " minuter kvar i fängelset!")
		...
		while jailTime > 0 do
			...
			for posId, v in pairs(Packages) do
				...
				if DistanceCheck <= 10.0 then
					...
					local PackageText = "Packetera"

					if not v["state"] then
						PackageText = "Redan tagen"
					end
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Du har redan tagit detta paket!")
							end
		...
		while true do
			...
			for p, v in pairs(Config.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "[E] för att öppna dörr", 0.4)
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "Packeterar... " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[E] Lämna paket", 0.4)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Fängsla närmaste person", value = "jail_closest_player" },
				{ label = "Ta ut någon ur fängelset", value = "unjail_player" }
			}
		}, 
	function(data, menu)
			...
			ESX.UI.Menu.Open(
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		ESX.ShowNotification("Tiden behöver vara i minuter!")
            	else
              		menu2.close()
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
						...
						ESX.UI.Menu.Open(
							...
							{
							  title = "Anledning till fängsling"
							},
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Du behöver skriva något här!")
						  	else
								menu3.close()
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Fängelset är tomt!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Fånge: " .. playerArray[i].name .. " | Tid kvar: " .. playerArray[i].jailTime .. " minuter", value = playerArray[i].identifier })
				end

				ESX.UI.Menu.Open(
					'default', GetCurrentResourceName(), 'jail_unjail_menu',
					{
						title = "Ta ut någon ur fängelset",
						align = "center",
						elements = elements
					},
				function(data2, menu2)
