			...
			if jailTime ~= nil then
				...
				if args[3] ~= nil then
					GetRPName(jailPlayer, function(Firstname, Lastname)
						TriggerClientEvent('chat:addMessage', -1, { args = { "[Polismyndigheten]",  Firstname .. " " .. Lastname .. " är nu i fängelse för anledning: " .. args[3] }, color = { 0, 0, 153 } })
					end)
				end
			else
				TriggerClientEvent("esx:showNotification", src, "Ogiltig tid!")
			end
		else
			TriggerClientEvent("esx:showNotification", src, "Detta ID är inte vaken!")
		end
	else
		TriggerClientEvent("esx:showNotification", src, "Du arbetar inte inom Polismyndigheten")
	end
end)
...
RegisterCommand("unjail", function(src, args)
	...
	if xPlayer["job"]["name"] == "police" then
		...
		if GetPlayerName(jailPlayer) ~= nil then
			...
			TriggerClientEvent("esx:showNotification", src, "Detta ID är inte vaket")
		end
	else
		TriggerClientEvent("esx:showNotification", src, "Du arbetar inte inom Polismyndigheten")
	end
...
AddEventHandler("esx-qalle-jail:jailPlayer", function(targetSrc, jailTime, jailReason)
	...
	GetRPName(targetSrc, function(Firstname, Lastname)
		TriggerClientEvent('chat:addMessage', -1, { args = { "[Polismyndigheten]",  Firstname .. " " .. Lastname .. " är nu i fängelse för: " .. jailReason .. " i " .. jailTime .. " dagar"  }, color = { 0, 0, 153 } })
	end)
end)
...
AddEventHandler("esx-qalle-jail:prisonWorkReward", function()
	...
	TriggerClientEvent("esx:showNotification", src, "Tack här har du lite pengar för mat!")
end)
