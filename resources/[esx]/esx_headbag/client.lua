...
if closestPlayer == -1 or closestDistance > 2.0 then 
    sendNotification('Finns ingen person i närheten', 'error', 2500)
else
  if not HaveBagOnHead then
    TriggerServerEvent('esx_worek:sendclosest', GetPlayerServerId(closestPlayer))
    sendNotification('Du sätter påsen', 'error', 2500)
    TriggerServerEvent('esx_worek:closest')
  else
    sendNotification('Denna person har redan en påse på huvudet', 'error', 2500)
  end
end
...
AddEventHandler('esx_worek:zdejmijc', function(gracz)
    sendNotification('Någon tog bort påsen från ditt huvud', 'error', 2500)
    DeleteEntity(Worek)
    SetEntityAsNoLongerNeeded(Worek)
    SendNUIMessage({type = 'closeAll'})
    HaveBagOnHead = false
...
function OpenBagMenu() --This function is menu function
    ...
    local elements = {
          {label = 'Sätt på påsen', value = 'puton'},
          {label = 'Ta bort påsen', value = 'putoff'},
