...
local FISH_WAITING_ILLEGALLY = 1
local FISH_HOOKED_ILLEGALLY = 2
local FISH_CAUGHT_ILLEGALLY = 3

local ACTION_FISHING = 'fishing'
local ACTION_FISHING_ILLEGALLY = 'fishing_illegally'
local ACTION_IN_FISHING_ZONE = 'in_fishing_zone'
local ACTION_IN_FISHING_ZONE_ILLEGALLY = 'in_fishing_zone_illegally'
local ACTION_SELLING_FISH = 'selling_fish'
local ACTION_SELLING_FISH_ILLEGALLY = 'selling_fish_illegally'
...
AddEventHandler('esx_fishing:playFishingAnimation', function()
	...
	CurrentActionMsg  = 'Håll ner ~y~E~w~ för att sluta fiska'
	FishHooked = FISH_WAITING
end)

RegisterNetEvent('esx_fishing:playFishingAnimationes')
AddEventHandler('esx_fishing:playFishingAnimationes', function()
	TaskStartScenarioInPlace(GetPlayerPed(-1), "WORLD_HUMAN_STAND_FISHING", 0, false)
	CurrentAction = ACTION_FISHING_ILLEGALLY
	CurrentActionMsg  = 'Håll ner ~y~E~w~ för att sluta ~r~tjuvfiska.'
	FishHooked = FISH_WAITING_ILLEGALLY
end)

AddEventHandler('esx_fishing:hasEnteredMarker', function(zone)
	if zone == 'FishingPosition1' or zone == 'FishingPosition2' or zone == 'FishingPosition3' or zone == 'FishingPosition4' or zone == 'FishingPosition5' or zone == 'FishingPosition6' or zone == 'FishingPosition14' or zone == 'FishingPosition15' or zone == 'FishingPosition16' or zone == 'FishingPosition17' or zone == 'FishingPosition18' or zone == 'FishingPosition19' or zone == 'FishingPosition20' or zone == 'FishingPosition21' or zone == 'FishingPosition22' or zone == 'FishingPosition23' or zone == 'FishingPosition24'  then
		CurrentActionMsg  = 'Tryck ~y~E~w~ för att fiska'
		CurrentAction = ACTION_IN_FISHING_ZONE
	end

	if zone == 'FishingPosition7' or zone == 'FishingPosition8' or zone == 'FishingPosition9' or zone == 'FishingPosition10' or zone == 'FishingPosition11' or zone == 'FishingPosition12' or zone == 'FishingPosition13' then
		CurrentActionMsg  = 'Tryck ~y~E~w~ för att ~r~tjuvfiska'
		CurrentAction = ACTION_IN_FISHING_ZONE_ILLEGALLY
	end

	if zone == 'SellFish' then
		CurrentActionMsg  = 'Tryck ~y~E~w~ för att sälja fisken'
		CurrentAction = ACTION_SELLING_FISH
	end
		if zone == 'SellFishIllegally' then
		CurrentActionMsg  = 'Tryck ~y~E~w~ för att sälja fisken'
		CurrentAction = ACTION_SELLING_FISH_ILLEGALLY
	end

	if zone == 'FishingLeaderboard' then
		CurrentActionMsg  = 'Tryck ~y~E~w~ för att se fiskerekord'
...
Citizen.CreateThread(function()
	while true do
		...
		while DisplayLeaderboard do
			local titleMessage = "Dagens fiskerekord"
...
Citizen.CreateThread(function()
	while true do
		...
		while FishHooked == FISH_HOOKED_ILLEGALLY and CurrentAction == ACTION_FISHING_ILLEGALLY do
			DrawScreenText('Tryck ~g~SPACE ~w~för att fånga fisk!', {255, 255, 255, 255, 1}, { 0.448, 0.3725 }, { 0.75, 0.75 }, true)
			DrawScreenRect({33, 33, 33, 150}, { 0.448, 0.3925 }, {0.25, 0.08})
			Wait(100)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlPressed(0,  Keys['E']) and CurrentAction == ACTION_IN_FISHING_ZONE_ILLEGALLY then
			TriggerServerEvent('esx_fishing:fishes')
			CurrentAction = 'tryfish'
			Wait(1000)
		end

		if IsControlPressed(0,  Keys['E']) and CurrentAction == ACTION_FISHING_ILLEGALLY then
			ClearPedTasksImmediately(GetPlayerPed(-1))
			CurrentAction = nil				
		end

		if IsControlPressed(0,  Keys['E']) and CurrentAction == ACTION_SELLING_FISH_ILLEGALLY then
			TriggerServerEvent('esx_fishing:sellAllFishes')
			CurrentAction = nil
		end
		...
		if CurrentAction == ACTION_FISHING_ILLEGALLY then
			Wait(50)

			if FishHooked == FISH_WAITING_ILLEGALLY then
				-- Waiting for a fish to bite, sleep 1 second and check if we got something
				Wait(1000)
				...
				if math.random(1, 600) < CurrentFishingOdds then
					FishHooked = FISH_HOOKED_ILLEGALLY
				end
			end

			if FishHooked == FISH_HOOKED_ILLEGALLY and IsControlPressed(0, Keys['SPACE']) then
				FishHooked = FISH_CAUGHT_ILLEGALLY

				ESX.TriggerServerCallback('esx_fishing:illegally', function(fish) 
					CaughtFish = fish
				end)
