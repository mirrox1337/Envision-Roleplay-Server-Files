...
function OpenSellMenu(veh, price, buyVehicle, owner)
	...
	if not buyVehicle then
		if price ~= nil then
			table.insert(elements, { ["label"] = "Byt pris - " .. price .. " :-", ["value"] = "price" })
			table.insert(elements, { ["label"] = "Ställ ut ditt fordon för försäljning", ["value"] = "sell" })
		else
		...
		if owner then
			table.insert(elements, { ["label"] = "Ta bort fordon från utförsäljning", ["value"] = "remove" })
		end
	end
