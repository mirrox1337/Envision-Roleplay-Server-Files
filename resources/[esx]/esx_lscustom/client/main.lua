		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					if isRimMod then
						price = math.floor(vehiclePrice * data.current.price / 250)
						TriggerServerEvent('esx_lscustom:buyMod', price)
					elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
						price = math.floor(vehiclePrice * v.price[data.current.modNum + 1] / 250)
						TriggerServerEvent('esx_lscustom:buyMod', price)
					elseif v.modType == 17 then
						price = math.floor(vehiclePrice * v.price[1] / 250)
						TriggerServerEvent('esx_lscustom:buyMod', price)
					else
						price = math.floor(vehiclePrice * v.price / 250)
						TriggerServerEvent('esx_lscustom:buyMod', price)
					end
				end
			...
			if v.modType then
				...
				if v.modType == 14 then -- HORNS
					for j = 0, 51, 1 do
						...
						if j == currentMods.modHorns then
							...
							price = math.floor(vehiclePrice * v.price / 250)
							_label = GetHornName(j) .. ' - <span style="color:green;">SEK ' .. price .. ' </span>'
						end
				...
				elseif v.modType == 'plateIndex' then -- PLATES
					for j = 0, 4, 1 do
						...
						if j == currentMods.plateIndex then
							...
							price = math.floor(vehiclePrice * v.price / 250)
							_label = GetPlatesName(j) .. ' - <span style="color:green;">SEK ' .. price .. ' </span>'
						end
						table.insert(elements, {label = _label, modType = k, modNum = j})
					end
				elseif v.modType == 22 then -- NEON
					...
					if currentMods.modXenon then
						...
						price = math.floor(vehiclePrice * v.price / 250)
						_label = _U('neon') .. ' - <span style="color:green;">SEK ' .. price .. ' </span>'
					end
				...
				elseif v.modType == 'neonColor' or v.modType == 'tyreSmokeColor' then -- NEON & SMOKE COLOR
					...
					for i=1, #neons, 1 do
						table.insert(elements, {
							label = '<span style="color:rgb(' .. neons[i].r .. ',' .. neons[i].g .. ',' .. neons[i].b .. ');">' .. neons[i].label .. ' - <span style="color:green;">SEK ' .. price .. '</span>',
							modType = k,
				...
				elseif v.modType == 'color1' or v.modType == 'color2' or v.modType == 'pearlescentColor' or v.modType == 'wheelColor' then -- RESPRAYS
					...
					for j = 1, #colors, 1 do
						local _label = ''
						price = math.floor(vehiclePrice * v.price / 250)
						_label = colors[j].label .. ' - <span style="color:green;">SEK ' .. price .. ' </span>'
				...
				elseif v.modType == 'windowTint' then -- WINDOWS TINT
					for j = 1, 5, 1 do
						...
						if j == currentMods.modHorns then
							...
							price = math.floor(vehiclePrice * v.price / 250)
							_label = GetWindowName(j) .. ' - <span style="color:green;">SEK ' .. price .. ' </span>'
						end
						table.insert(elements, {label = _label, modType = k, modNum = j})
					end
					...
					for j = 0, modCount, 1 do
						...
						if modName then
							...
							if j == currentMods.modFrontWheels then
								...
								price = math.floor(vehiclePrice * v.price / 250)
								_label = GetLabelText(modName) .. ' - <span style="color:green;">SEK ' .. price .. ' </span>'
							end
				...
				elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
					...
					for j = 0, modCount, 1 do
						...
						if j == currentMods[k] then
							...
							price = math.floor(vehiclePrice * v.price[j+1] / 250)
							_label = _U('level', j+1) .. ' - <span style="color:green;">SEK ' .. price .. ' </span>'
						end
						table.insert(elements, {label = _label, modType = k, modNum = j})
				...
				elseif v.modType == 17 then -- TURBO
					...
					if currentMods[k] then
						...
						_label = 'Turbo - <span style="color:green;">SEK ' .. math.floor(vehiclePrice * v.price[1] / 250) .. ' </span>'
					end
					table.insert(elements, {label = _label, modType = k, modNum = true})
				else
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (PlayerData.job and PlayerData.job.name == 'bennys') or not Config.IsMechanicJobOnly then
				for k,v in pairs(Config.Zones) do
