...
Citizen.CreateThread(function()
  while true do
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'police' and (GetGameTimer() - GUI.Time) > 150 and isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastPad ~= currentPad or LastAction ~= currentAction)) and PlayerData.job ~= nil and PlayerData.job.name == 'police' then
...
Citizen.CreateThread(function()
  ...
  local playerPed = GetPlayerPed(-1)
    ...
    for pad,padData in pairs(Config.Pads) do  
      if GetDistanceBetweenCoords(coords, padData.Marker.x,  padData.Marker.y, padData.Marker.z,  true) < Config.DrawDistance and PlayerData.job ~= nil and PlayerData.job.name == 'police' then
