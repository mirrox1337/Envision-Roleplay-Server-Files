			...
			if distance < maxDistance then
				...
				if IsControlJustReleased(0, Keys['E']) then
					...
					if isAuthorized then
						...
						ESX.ShowNotification("Du har inga nycklar till den här dörren.")
					end
				end
			end
		end
...
function ApplyDoorState(doorID)
	...
	if doorID.locked then
		if not doorID.heading == false then
			SetEntityHeading(closeDoor, doorID.heading)
		end
	end
end
...
function Draw3DText(x, y, z, text)
    ...
    if onScreen then
        ...
        DrawText(_x,_y)
	local factor = (string.len(text)) / 850
