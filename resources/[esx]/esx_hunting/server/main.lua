...
AddEventHandler('esx-qalle-hunting:sell', function()
    ...
    if MeatQuantity > 0 or LeatherQuantity > 0 then
        ...
        TriggerClientEvent('esx:showNotification', xPlayer.source, 'Du sålde ' .. LeatherQuantity + MeatQuantity .. ' och tjänade' .. LeatherPrice * LeatherQuantity + MeatPrice * MeatQuantity)
    else
        TriggerClientEvent('esx:showNotification', xPlayer.source, 'Du har inget att sälja')
    end
