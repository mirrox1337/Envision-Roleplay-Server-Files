...
OpenInviteMenu = function(motelRoomData)
    ...
    if #closestPlayers <= 0 then
        return exports['mythic_notify']:SendAlert('error', ('Ingen är utanför din dörr!')) --ESX.ShowNotification("Ingen är utanför din dörr")
    end
    ...
    ESX.TriggerServerCallback("james_motels:retreivePlayers", function(playersRetreived)
        if playersRetreived then
            ...
            --ESX.ShowNotification("Det gick inte att hämta information om männsikan", "error", 3500)
            exports['mythic_notify']:SendAlert('error', ('Det gick inte att hämta information om personen!'))
        end
    end, closestPlayers)
end
...
OpenLandLord = function()
    ...
    if ownedMotel then 
        table.insert(menuElements, {
            ["label"] = "Sälj ditt motellrum (" .. Config.MotelPrice / 10 .. ":-)",
            ["action"] = "sell"
        })
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "main_motel_landlord", {
        ...
        elseif action == "sell" then
            ESX.TriggerServerCallback("james_motels:sellMotel", function(sold)
                if sold then
                    exports['mythic_notify']:SendAlert('inform', ('Du sålde ditt motellrum')) --ESX.ShowNotification("Du sålde ditt motellrum")
                end
            end, ownedMotel)
        elseif action == "buy_key" then
            ESX.TriggerServerCallback("james_motels:checkMoney", function(approved)
                if approved then
                    ...
                    exports['mythic_notify']:SendAlert('error', ('Du har ej tillräckligt med pengar för att betala.')) --ESX.ShowNotification("Du har ej tillräckligt med pengar för att betala.")
                end
            end)
        end
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "main_accept_motel", {
        ...
        if action == "yes" then
            ESX.TriggerServerCallback("james_motels:buyMotel", function(bought, uuid)
                if bought then
                    exports['mythic_notify']:SendAlert('success', ('Du köpte rum #' .. motelRoom)) --ESX.ShowNotification("Du köpte rummet #" .. motelRoom)

                    AddKey({
                        ["id"] = "motel-" .. uuid,
                        ["label"] = "Motellrummet #" .. motelRoom .. " - " .. ESX.PlayerData["character"]["firstname"]
                    })
                else
                    exports['mythic_notify']:SendAlert('error', ('Du har inte råd med det här rummet!')) --ESX.ShowNotification("Du har inte råd med det här rummet!")
                end
		...
		ESX.UI.Menu.Open("default", GetCurrentResourceName(), "motel_main_dressing_menu", {
			...
			TriggerEvent("skinchanger:getSkin", function(skin)
                ESX.TriggerServerCallback("james_motels:getPlayerOutfit", function(clothes)
                    ...
                    --ESX.ShowNotification("Du satte på dig andra kläder!")
                    exports['mythic_notify']:SendAlert('inform', ('Du satte på dig andra kläder!'))
                end, currentOutfit)
			end)
        end, function(menuData, menuHandle)
			menuHandle.close()
    ...
    if insideMotel then
        ...
        if ownedMotel then
            ...
            ESX.Game.Teleport(PlayerPedId(), Config.LandLord["position"], function()
                exports['mythic_notify']:SendAlert('inform', ('Du somnade och blev utskickad ifrån ditt rum!')) --ESX.ShowNotification("Du somnade och blev utskickad ifrån ditt rum!")
            end)
        end
    end
end
...
WaitForModel = function(model)
    if not IsModelValid(model) then
        exports['mythic_notify']:SendAlert('error', ('This model doesnt exist.')) --ESX.ShowNotification("This model doesn't exist.")
