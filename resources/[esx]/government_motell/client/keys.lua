...
AddEventHandler("james_motels:keyTransfered", function(keyData)
    ...
    exports['mythic_notify']:SendAlert('inform', ('Du fick en nyckel!')) --ESX.ShowNotification("Du fick en nyckel", "error", 3500)
end)
...
AddKey = function(keyData)
    ...
    ESX.TriggerServerCallback("james_motels:addKey", function(added)
        if added then
            ...
            exports['mythic_notify']:SendAlert('inform', ('Du tog emot din nyckel!')) --ESX.ShowNotification("Du tog emot din nyckel!", "warning", 3500)
        end
    end, keyData)
end
        ...
        if keyData["uuid"] == keyUUID then
            ESX.TriggerServerCallback("james_motels:removeKey", function(removed)
                if removed then
                    ...
                    exports['mythic_notify']:SendAlert('error', ('Nyckel togs bort!')) --ESX.ShowNotification("Nyckel togs bort", "error", 3500)
                end
            end, keyUUID)
        ...
        if keyData["uuid"] == currentKeyData["uuid"] then
            ESX.TriggerServerCallback("james_motels:transferKey", function(removed)
                if removed then
                    ...
                    exports['mythic_notify']:SendAlert('inform', ('Nyckel skickad!')) --ESX.ShowNotification("Key sent.", "error", 3500)
                end
...
ShowKeyMenu = function()
    ...
    if #cachedData["keys"] == 0 then return exports['mythic_notify']:SendAlert('error', ('Du har inga nycklar på dig')) --[[ESX.ShowNotification("Du har inga nycklar på dig", "error", 3000) --]] end
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "key_main_menu", {
        ...
        ConfirmGiveKey(currentKey, function(confirmed)
            if confirmed then
                ...
                DrawBusySpinner("Lämnar över nyckeln")
...
ConfirmGiveKey = function(keyData, callback)
    ...
    if closestPlayer ~= -1 and closestPlayerDistance > 3.0 then
        return exports['mythic_notify']:SendAlert('error', ('Ingen i närheten')) --ESX.ShowNotification("Du är inte nära en person.")
    end
