...
Citizen.CreateThread(function()
	...
	while true do
		Citizen.Wait(1) 
		RemoveBlip(blip)
		if ( IsDisabledControlPressed( 1, Keys['LEFTCTRL'] ) and IsDisabledControlJustPressed( 1, Keys['NENTER'] ) ) then 
			if shown == true then 
				shown = false 
				SendNUIMessage({
					radar 		= shown, 
		...
		if shown == true then
			if ( IsDisabledControlPressed( 1, Keys['LEFTCTRL'] ) and IsDisabledControlJustPressed( 1, Keys['N+'] ) ) then 
				SendNUIMessage({
					type 	= 'limit', 
					status 	= 'up'
				})
			...
			if ( IsDisabledControlPressed( 1, Keys['LEFTCTRL'] ) and IsDisabledControlJustPressed( 1, Keys['N9'] ) ) then 
			...
			if shown then	
				...
				if ( IsPedSittingInAnyVehicle( ped ) ) then 
					...
					if ( GetPedInVehicleSeat( vehicle, -1 ) == ped and GetVehicleClass( vehicle ) == 18 ) then 
						...
						local carM = GetClosestVehicle(pos['x'], pos['y'], pos['z'], 10.0,0,60)--15 70
