			...
			for _,v in pairs(ownedCars) do
				if Config.UseVehicleNamesLua then
					...
					if Config.ShowVehicleLocation then
						if v.stored then
							labelvehicle = '<span style="color:#771282">' ..vehicleName.. '  |  <span style="color:gray">' ..plate.. '</span>  |  <span style="color:green">Parkerad</span>'
						else
							labelvehicle = '<span style="color:#771282">' ..vehicleName.. '  |  <span style="color:gray">' ..plate.. '</span>  |  <span style="color:red">Ute</span>'
						end
					else
					if v.stored then
...
function reparation(apprasial, vehicle, vehicleProps)
	...
	local elements = {
		{label = "Parkera", value = 'yes'},
		{label = "Kontakta Mekaniker", value = 'no'},
	}
	
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'delete_menu', {
		--title    = _U('damaged_vehicle'),
		title    = 'Ditt fordon är skadat',
		align    = 'center',
		elements = elements
	}, function(data, menu)
		menu.close()
		...
		if Config.UseCarGarages then
			for k,v in pairs(Config.CarGarages) do
				if (GetDistanceBetweenCoords(coords, v.GaragePoint.x, v.GaragePoint.y, v.GaragePoint.z, true) < Config.DrawDistance) then
					...
					--Marker("~w~[~g~E~w~] ~g~För att öppna garage", 27, v.GaragePoint.x, v.GaragePoint.y, v.GaragePoint.z)
					...
					--Marker("~w~[~g~E~w~] ~r~För att parkera fordon", 27, v.DeletePoint.x, v.DeletePoint.y, v.DeletePoint.z)
				end
			end
			...
			for k,v in pairs(Config.CarPounds) do
				if (GetDistanceBetweenCoords(coords, v.PoundPoint.x, v.PoundPoint.y, v.PoundPoint.z, true) < Config.DrawDistance) then
					...
					Marker("~w~[~g~E~w~] ~y~För att lösa ut ditt fordon ~g~" ..Config.CarPoundPrice.. "kr", 27, v.PoundPoint.x, v.PoundPoint.y, v.PoundPoint.z)
				end
			end
		end
...
--RemoveObject
local object = prop_sec_barrier_Id_01a
	DeleteObject(object)