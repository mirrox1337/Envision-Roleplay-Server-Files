...
Citizen.CreateThread(function()
	...
	while true do
		...
		local crParking = nil
		for k, v in pairs(Config.ParkingLocations) do
			if GetDistanceBetweenCoords(pl.x, pl.y, pl.z, v.x, v.y, v.z, true) < v.size + 100.0 then
				inParking = true
				crParking = k
			end
		end
	...
	while true do
		...
		for k, v in pairs(Config.ParkingLocations) do
			if GetDistanceBetweenCoords(pl.x, pl.y, pl.z, v.enter.x, v.enter.y, v.enter.z, true) < 20 then
				Draw3DText(v.enter.x, v.enter.y, v.enter.z, v.name, 4, 0.2, 0.2)
				Draw3DText(v.enter.x, v.enter.y, v.enter.z - 0.5, string.format(_U("parking_fee", v.fee)), 4, 0.1, 0.1)
			end
		end
		for k, v in pairs(LocalVehicles) do
			if GetDistanceBetweenCoords(pl.x, pl.y, pl.z, v.data.location.x, v.data.location.y, v.data.location.z, true) < 3.0 then
				Draw3DText(v.data.location.x, v.data.location.y, v.data.location.z + 0.2, string.format(_U("owner", v.name)), 4, 0.08, 0.12)
				Draw3DText(v.data.location.x, v.data.location.y, v.data.location.z, string.format(_U("plate", v.plate)), 4, 0.05, 0.08)
				Draw3DText(v.data.location.x, v.data.location.y, v.data.location.z - 0.2, string.format(_U("fee", v.fee)), 4, 0.05, 0.08)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	for k, v in pairs(Config.ParkingLocations) do
		...
		SetBlipSprite(tempBlip, 357) --524
		SetBlipColour(tempBlip, 67)
				...
				if storedVehicle ~= false then
					...
					ESX.TriggerServerCallback("esx_realparking:driveCar", function(callback)
						if callback.status then
							...
							exports['mythic_notify']:SendAlert('inform', (callback.message))
						else
							--ESX.ShowNotification(callback.message)
							exports['mythic_notify']:SendAlert('inform', (callback.message))
						end
					...
					if veh ~= 0 then
						if IsThisModelACar(GetEntityModel(veh)) or IsThisModelABike(GetEntityModel(veh)) then
							...
							exports['mythic_notify']:SendAlert('inform', (_U("only_allow_car")))
						end
					end
				end
			end
