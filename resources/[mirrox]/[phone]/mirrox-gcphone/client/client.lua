...
AddEventHandler('qalle:job', function(number, message, position, caller, dispatchRequestId)
  ...
  elseif number == 'Akuten' then
    numbertext = 'Akuten'
  elseif number == 'Räddningstjänsten' then
    numbertext = 'Räddningstjänsten'
  ...
  elseif number == 'forsakring' then
    numbertext = 'forsakring' 
  elseif number == 'nightclub' then
    numbertext = 'nightclub' 
  elseif number == 'lawyer' then
    numbertext = 'lawyer' 
  end
      ...
      if IsControlJustReleased(0, Keys['Y']) then
        ...
        if CurrentAction == 'dispatch' then
          ...
          elseif CurrentActionData.phoneNumber == 'Polisen' then
            ...
            elseif CurrentActionData.phoneNumber == 'Räddningstjänsten' then
              ESX.TriggerServerCallback('gcPhone:getIdentity', function (identity)
                TriggerServerEvent('gcPhone:stopDispatch2', CurrentDispatchRequestId)
              SetNewWaypoint(CurrentActionData.position.x,  CurrentActionData.position.y)
              TriggerServerEvent('gcPhone:sendMessage', CurrentActionData.caller, 'SOS Operatör: Räddningstjänsten är på väg mot din angivna larmposition. Vid skada på andra part, kontrollera så personen har puls och fria luftvägar. Om inte, påbörja hjärt- och lungräddning!') 
              end)
            elseif CurrentActionData.phoneNumber == 'Taxi' or CurrentActionData.phoneNumber == 'Mekonomen' or CurrentActionData.phoneNumber == 'Securitas' then
              ESX.TriggerServerCallback('gcPhone:getIdentity', function (identity)
            ...
            elseif CurrentActionData.phoneNumber == 'Bilhandlaren' or CurrentActionData.phoneNumber == 'Bennys' or CurrentActionData.phoneNumber == 'forsakring' or CurrentActionData.phoneNumber == 'nightclub' or CurrentActionData.phoneNumber == 'lawyer' then
                ESX.TriggerServerCallback('gcPhone:getIdentity', function (identity)
                  TriggerServerEvent('gcPhone:stopDispatch', CurrentDispatchRequestId)
...
function startFixeCall (fixeNumber)
  local number = ''
  AddTextEntry('FMMC_KEY_TIP1', 'Skriv ett nummer')
  ...
  while true do 
    ...
    for i, _ in pairs(PhoneInCall) do 
        ...
        if (dist <= soundDistanceMax) then
          DrawMarker(28, PhoneInCall[i].coords.x, PhoneInCall[i].coords.y, PhoneInCall[i].coords.z,
              0,0,0, 0,0,0, 0.1,0.1,0.1, 48,209,88,100, 0,0,0,0,0,0,0)
...
--  Inventory lock/unlock based on phone status
Citizen.CreateThread(function()
  while true do
  Citizen.Wait(5000)
    if menuIsOpen == false then
      exports['disc-inventoryhud']:phoneUnlock()
    end
  end
end)
...
function TooglePhone() 
  ...
  if menuIsOpen == true then 
    exports['disc-inventoryhud']:phoneLock()
    PhonePlayIn()
  else
    exports['disc-inventoryhud']:phoneUnlock()
    PhonePlayOut()
