...
TriggerEvent('es:addGroupCommand', 'adminme', 'admin', function(source, args, user)
	local player = tonumber(args[1])
	local meddelande = args
	
	if player then
		TriggerClientEvent('slizzarn:adminme', player, meddelande)
	else
		TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Ogiltigt ID.")
	end
end, function(source, args, user)
	--TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
end)
...
TriggerEvent('es:addGroupCommand', 'setmoney', 'admin', function(source, args, user)
	...
	DiscordMSG("ABUSE LOG:", GetPlayerName(source) .. " gav " .. money_amount .. " till " .. xPlayer.name)
...
TriggerEvent('es:addGroupCommand', 'giveitem', 'admin', function(source, args, user)
	...
	if count ~= nil then
		if xPlayer.getInventoryItem(item) ~= nil then
			...
			--DiscordMSG("ABUSE LOG:", GetPlayerName(source) .. " gav " .. count .. " " .. item .. " till " .. xPlayer.name)
		else
...
TriggerEvent('es:addGroupCommand', 'rensa', 'user', function(source, args, user)
	TriggerClientEvent('chat:clear', source)
end, function(source, args, user)
	--TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
end, {help = _U('chat_clear')})

TriggerEvent('es:addGroupCommand', 'rensaallt', 'admin', function(source, args, user)
...
discordMSG = function(title, msg)
	...
	PerformHttpRequest('https://discordapp.com/api/webhooks/638409531243888671/K-kqrK30top7qLEoMo-pLDut3Bc5PGOH6jOksYESZJ0R7eNvgXs7ukWtkaWp6OvZDBIE', function(err, text, headers) end, 'POST', json.encode({username = "Bokföring", embeds = embeds}), { ['Content-Type'] = 'application/json' })
end