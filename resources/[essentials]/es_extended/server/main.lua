AddEventHandler('es:playerLoaded', function(source, _player)
	...
	TriggerEvent('es:getPlayerFromId', _source, function(player)
		...
		table.insert(tasks, function(cb)
			MySQL.Async.fetchAll('SELECT firstname, lastname FROM `users` WHERE `identifier` = @identifier', {
				...
				if response and #response > 0 then
					userData.character["firstname"] = response[1]["firstname"]
					userData.character["lastname"] = response[1]["lastname"]
				end
		...
		Async.parallel(tasks, function(results)
			local xPlayer = CreateExtendedPlayer(player, userData.accounts, userData.inventory, userData.job, userData.loadout, userData.playerName, userData.lastPosition, userData.character)
	...
	if type == 'item_standard' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				local pickupLabel = ('%s %s'):format(itemCount, xItem.label)
	...
	elseif type == 'item_money' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > playerCash or playerCash < 1) then
				...
				local pickupLabel = ('%s'):format(_U('locale_currency', ESX.Math.GroupDigits(itemCount)), _U('cash'))
	...
	elseif type == 'item_account' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > account.money or account.money < 1) then
				...
				local pickupLabel = ('%s %s'):format(_U('locale_currency', ESX.Math.GroupDigits(itemCount)), account.label)
...
AddEventHandler('esx:onPickup', function(id)
	...
	if pickup.type == 'item_standard' then
		...
		if remaining > 0 then
			...
			local pickupLabel = ('%s %s'):format(remaining, item.label)
			
			('item_standard', pickup.name, remaining, pickupLabel, _source)
		end
