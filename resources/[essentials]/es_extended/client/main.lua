...
AddEventHandler('esx:addInventoryItem', function(item, count)
	...
	--ESX.ShowNotification('Du fick <span style="color:yellow">' .. count .. ' ' .. item.label .. '</span>')
...
AddEventHandler('esx:removeInventoryItem', function(item, count)
	...
	--if not item["name"] == "9mm" and not item["name"] == "7mm" not item["name"] == "5mm" then
		--ESX.ShowNotification('Du förlorade <span style="color:yellow">' .. count .. ' ' .. item.label .. '</span>')
	--end
...
AddEventHandler('esx:pickup', function(type, id, label, player)
	...
	ESX.Game.SpawnLocalObject(prop, {
		x = x,
		y = y,
		z = z - 1.0,
	}, function(obj)
...
Citizen.CreateThread(function()
	while true do
	  ...
	  for k,v in pairs(Pickups) do
		...
		if distance <= 1.0 and not v.inRange and IsControlPressed(0, Keys["E"]) then
