...
AddEventHandler('playerConnecting', function(user, set)
	for k,v in ipairs(GetPlayerIdentifiers(source))do
		if isBanned(v) then
			set(GetConvar("es_admin_banreason", "Du är bannad från denna server..."))
			CancelEvent()
			break
		end
	end
	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			if available or user.getGroup() == "superadmin" then
				...
				TriggerClientEvent('chatMessage', Source, "", {255, 0, 0}, "You do not have permission to do this")
			end
		end)
	end)
end)
				...
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget and available then
						...
						if type == "ban" then
							...
							DropPlayer(id, GetConvar("es_admin_banreason", "Du blev bannad från servern."))
						end
					else
						if not available then
							TriggerClientEvent('chatMessage', Source, '', {255, 0, 0}, "Your group can not use this command.")
						else
		...
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			...
			if t == "group" then
				if(GetPlayerName(USER) == nil)then
					TriggerClientEvent('chatMessage', source, '', {255, 0, 0}, "Player not found")
				else
					TriggerEvent("es:getAllGroups", function(groups)
						if(groups[GROUP])then
							TriggerEvent("es:setPlayerData", USER, "group", GROUP, function(response, success)
								...
								TriggerClientEvent('chatMessage', -1, "", {0, 0, 0}, "Group of ^2^*" .. GetPlayerName(tonumber(USER)) .. "^r^0 has been set to ^2^*" .. GROUP)
							end)
						else
							TriggerClientEvent('chatMessage', source, '', {255, 0, 0}, "Group not found")
						end
					end)
				end
			elseif t == "level" then
				if(GetPlayerName(USER) == nil)then
					...
					if(GROUP ~= nil and GROUP > -1)then
						TriggerEvent("es:setPlayerData", USER, "permission_level", GROUP, function(response, success)
							if(true)then
								TriggerClientEvent('chatMessage', -1, "", {0, 0, 0}, "Permission level of ^2" .. GetPlayerName(tonumber(USER)) .. "^0 has been set to ^2 " .. tostring(GROUP))
							end
						end)	
					else
						TriggerClientEvent('chatMessage', source, '', {255, 0, 0}, "Invalid integer entered")
					end
				end
			elseif t == "money" then
				if(GetPlayerName(USER) == nil)then
			...
			elseif t == "bank" then
				...
				TriggerClientEvent('chatMessage', source, '', {255, 0, 0}, "superadmin required to do this")
			end
		end)
	end)	
end)
...
end, {help = "Skicka meddelande till hela servern", params = {{name = "announce", help = "Meddelande att skicka till alla"}}})
	...
	if commandName == 'setadmin' then
		...
		TriggerEvent("es:setPlayerData", tonumber(args[1]), "permission_level", tonumber(args[2]), function(response, success)
			...
			if(true)then
				...
				TriggerClientEvent('chatMessage', -1, "", {0, 0, 0}, "Permission level of ^2" .. GetPlayerName(tonumber(args[1])) .. "^0 has been set to ^2 " .. args[2])
			end
		end)
			...
			if(groups[args[2]])then
				TriggerEvent("es:setPlayerData", tonumber(args[1]), "group", args[2], function(response, success)
					...
					if(true)then
						...
						TriggerClientEvent('chatMessage', -1, "", {0, 0, 0}, "Group of ^2^*" .. GetPlayerName(tonumber(args[1])) .. "^r^0 has been set to ^2^*" .. args[2])
					end
				end)
			else
	...
	elseif commandName == 'giverole' then
		...
		if(GetPlayerName(tonumber(args[1])) == nil)then
			...
			TriggerEvent("es:getPlayerFromId", tonumber(args[1]), function(user)
				...
				TriggerClientEvent("chatMessage", user.get('source'), "", {255, 0, 0}, "You've been given a role: ^2" .. table.concat(args, " "))
			end)
	...
	elseif commandName == 'removerole' then
		...
		if(GetPlayerName(tonumber(args[1])) == nil)then
			...
			TriggerEvent("es:getPlayerFromId", tonumber(args[1]), function(user)
				...
				TriggerClientEvent("chatMessage", user.get('source'), "", {255, 0, 0}, "A role was removed: ^2" .. table.concat(args, " "))
			end)
	...
	elseif commandName == 'setmoney' then
			...
			TriggerEvent("es:getPlayerFromId", tonumber(args[1]), function(user)
				if(user)then
					...
					RconPrint("Money set")
					TriggerClientEvent('chatMessage', tonumber(args[1]), "", {0, 0, 0}, "Your money has been set to: ^2^*$" .. tonumber(args[2]))
				end
			end)
...
TriggerEvent('es:addCommand', 'admin', function(source, args, user)
	TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "Level: ^*^2 " .. tostring(user.get('permission_level')))
	TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "Group: ^*^2 " .. user.getGroup())
	...
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				if(#reason == 0)then
					reason = "Sparkad: Du blev sparkad från servern."
				else
					reason = "Sparkad: " .. table.concat(reason, " ")
				end

				TriggerClientEvent('chatMessage', -1, "", {255, 0, 0}, "Player ^2" .. GetPlayerName(player) .. "^0 has been kicked(^2" .. reason .. "^0)")
	...
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chatMessage', player, "", {255, 0, 0}, "You have been " .. state .. " by ^2" .. GetPlayerName(source))
				TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "Player ^2" .. GetPlayerName(player) .. "^0 has been " .. state)
			end)
		else
			TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "Incorrect player ID!")
		end
	...
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chatMessage', player, "", {255, 0, 0}, "You have brought by ^2" .. GetPlayerName(source))
				TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "Player ^2" .. GetPlayerName(player) .. "^0 has been brought")
			end)
		else
			TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "Incorrect player ID!")
		end
	...
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chatMessage', player, "", {255, 0, 0}, "You have slapped by ^2" .. GetPlayerName(source))
				TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "Player ^2" .. GetPlayerName(player) .. "^0 has been slapped")
			end)
		else
			TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "Incorrect player ID!")
		end
		...
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				if(target)then
					...
					TriggerClientEvent('chatMessage', player, "", {255, 0, 0}, "You have been teleported to by ^2" .. GetPlayerName(source))
					TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "Teleported to player ^2" .. GetPlayerName(player) .. "")
				end
			end)
		else
			TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "Incorrect player ID!")
...
-------me troll
local frozen = {}
TriggerEvent('es:addGroupCommand', 'adminmenu', "admin", function(source, args, user)
	TriggerClientEvent('slizzarn:admin', source)
end, {help = "Kul att du provar iaf", params = {{name = "userid", help = "The ID of the player"}}})

RegisterServerEvent("nxrp-admin:handleAction")
AddEventHandler("nxrp-admin:handleAction", function(target, handle, data)
	TriggerClientEvent("nxrp-admin:handleAction", target, handle, data)
end)
	...
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chatMessage', player, "", {255, 0, 0}, "You have been killed by ^2" .. GetPlayerName(source))
				TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "Player ^2" .. GetPlayerName(player) .. "^0 has been killed.")
			end)
		else
			TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "Incorrect player ID!")
		end
	...
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "Player ^2" .. GetPlayerName(player) .. "^0 has been crashed.")
			end)
		else
			TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "Incorrect player ID!")
		end
...
TriggerEvent('es:addGroupCommand', 'pos', "owner", function(source, args, user)
	TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "This command was removed")
end, function(source, args, user)
	TriggerClientEvent('chatMessage', source, "", {255, 0, 0}, "This command was removed")
