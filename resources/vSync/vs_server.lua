...
AddEventHandler('es_wsync:freeze_time', function(source)
	...
	if source ~= 0 then
		...
		if freezeTime then
			TriggerClientEvent('esx:showNotification', source, 'Tiden är nu frozen.')
		else
			TriggerClientEvent('esx:showNotification', source, 'Tid är inte längre fryst frozen.')
		end
	else
		freezeTime = not freezeTime
		if freezeTime then
...
AddEventHandler('es_wsync:freeze_weather', function(source)
	if source ~= 0 then
		...
		if not DynamicWeather then
			TriggerClientEvent('esx:showNotification', source, 'Dynamic weather changes are now disabled.')
		else
			TriggerClientEvent('esx:showNotification', source, 'Dynamic weather changes are now enabled.')
		end
	else
		DynamicWeather = not DynamicWeather
		if not DynamicWeather then
...
AddEventHandler('es_wsync:blackout', function(source)
	if source == 0 then
		...
		blackout = not blackout
		if blackout then
			TriggerClientEvent('esx:showNotification', source, 'Blackout is now enabled.')
		else
			TriggerClientEvent('esx:showNotification', source, 'Blackout is now disabled.')
		end
		TriggerEvent('es_wsync:requestSync')
	end
end)
...
AddEventHandler('es_wsync:morning', function(source)
	...
	ShiftToMinute(0)
	ShiftToHour(9)
	TriggerClientEvent('esx:showNotification', source, 'Time set to morning.')
	TriggerEvent('es_wsync:requestSync')
end)
...
AddEventHandler('es_wsync:noon', function(source)
	...
	ShiftToMinute(0)
	ShiftToHour(12)
	TriggerClientEvent('esx:showNotification', source, 'Time set to noon.')
	TriggerEvent('es_wsync:requestSync')
end)
...
AddEventHandler('es_wsync:evening', function(source)
	...
	ShiftToMinute(0)
	ShiftToHour(18)
	TriggerClientEvent('esx:showNotification', source, 'Time set to evening.')
	TriggerEvent('es_wsync:requestSync')
end)
...
AddEventHandler('es_wsync:night', function(source)
	...
	ShiftToMinute(0)
	ShiftToHour(23)
	TriggerClientEvent('esx:showNotification', source, 'Time set to night.')
	TriggerEvent('es_wsync:requestSync')
end)
...
TriggerEvent('es:addGroupCommand', 'time', 'admin', function(source, args, user)
	...
	elseif source ~= 0 then
		if tonumber(args[1]) ~= nil and tonumber(args[2]) ~= nil then
			...
			TriggerClientEvent('esx:showNotification', source, 'Time was changed to: ' .. newtime .. "!")
			TriggerEvent('es_wsync:requestSync')
		else
