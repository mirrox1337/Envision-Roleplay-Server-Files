			...
			if dstCheck <= 2.5 then 
				...
				if display then
					...
					if dstCheck <= 1.0 then
						local displayText = isCracking and "Dyrkar..." or "~INPUT_CONTEXT~ Dyrka låset."
						...
						--if IsControlJustReleased(0, 38) then
							if IsControlJustPressed(0, 38) then
								...
								for i=1, #inventory, 1 do                          
									if inventory[i].name == "lUbait" then
										LockpickAmount = inventory[i].count
									end
								end
					
								if LockpickAmount >= 1 then
									TriggerServerEvent('esx_phone:send', 'police', 'Någon bryter sig in i ett kassaskåp, åk till platsen och säkra den.', false, { x = coords.x, y = coords.y, z = coords.z })
