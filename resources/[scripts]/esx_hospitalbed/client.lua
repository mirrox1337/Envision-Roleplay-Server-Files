        ...
        if IsPedDeadOrDying(me) then
            ...
            for k, v in pairs(receptionPeds) do
                if DoesEntityExist(v) then
                    if GetDistanceBetweenCoords(GetEntityCoords(me), GetOffsetFromEntityInWorldCoords(v, 0.0, 2.0, 0.0), true) <= 3.5 then 
...
AddEventHandler('esx_beds:revive', function(hospital)
    ...
    while timer >= GetGameTimer() do for i = 0, 256 do DisableAllControlActions(i) end Wait(0) end
    ...
    if DoesEntityExist(ped) then
        ...
        while timer >= GetGameTimer() do 
            ...
            for i = 0, 256 do DisableAllControlActions(i) end
