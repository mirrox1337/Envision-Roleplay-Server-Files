	...
	if dna.p == nil then
		...
		if IsPedFatallyInjured(ped) then
			...
			if weapon ~= nil then
				...
				ESX.ShowNotification('Tog DNA från den döda kroppen..')
			else
				ESX.ShowNotification("Kunde inte hitta DNA..")
			end
		else
			dna = {k = nil, d = nil, w = nil, p = GetPlayerServerId(player)}
			ESX.ShowNotification('Tog personens DNA..')
		end
	else
		ESX.ShowNotification('Du kan max ha 1 prov på dig, lägg in i databasen..')
	end
end)
