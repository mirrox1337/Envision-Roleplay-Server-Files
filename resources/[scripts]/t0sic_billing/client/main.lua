    ...
    ESX.TriggerServerCallback("t0sic-billing:fetchBillings", function(data)
        for i = 1, #data, 1 do
            table.insert(billings, {
                ["label"]           = 'Faktura - <span style="color:yellow"> ' .. data[i]["id"] .. '</span> - Datum: <span style="color:yellow">' .. data[i]["date"] .. '</span>',
                ["date"]            = data[i]["date"],
                ["id"]              = data[i]["id"],
                ["price"]           = data[i]["amount"],
...
DeleteBilling = function(invoice)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'delete_billing',
        {
            title    = 'Avbryt faktura - <span style="color:yellow"> ' .. invoice["id"] .. '</span>',
            align    = 'right',
            elements = {
                { ["label"] = "Ja" },
                { ["label"] = "Nej" }
