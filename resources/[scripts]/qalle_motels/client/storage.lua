            ...
            if action["type"] == "weapon" then
                ...
                ESX.UI.Menu.Open("dialog", GetCurrentResourceName(), "main_storage_count", {
                    ...
                    if not newCount then
                        return ESX.ShowNotification("Vänligen skriv ett nummer.")
    ...
    if Config.DirtyMoney then
        ...
        for accountIndex, accountData in pairs(playerAccounts) do
            if accountData["name"] == "black_money" then
                ...
                table.insert(menuElements, {
                    ["label"] = accountData["label"] .. " - " .. accountData["count"] .. " SEK",
                    ["action"] = accountData
                })
            end
        end
...
AddItemToStorage = function(storageId, newItem)
    ...
    ESX.TriggerServerCallback("qalle_motels:addItemToStorage", function(updated)
        if updated then
            ESX.ShowNotification("Du la in " .. newItem["count"] .. "st - " .. newItem["label"])
        end
...
TakeItemFromStorage = function(storageId, newItem)
    ...
    ESX.TriggerServerCallback("qalle_motels:takeItemFromStorage", function(updated)
        if updated then
            ESX.ShowNotification("Du tog ut " .. newItem["count"] .. "st - " .. newItem["label"])
        end
    end, cachedData["storages"][storageId], newItem, storageId)
end