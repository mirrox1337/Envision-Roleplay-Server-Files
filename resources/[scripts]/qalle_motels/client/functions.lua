...
OpenInviteMenu = function(motelRoomData)
    ...
    local closestPlayers = ESX.Game.GetPlayersInArea(Config.MotelsEntrances[motelRoomData["room"]], 7.0)--5.0
...
OpenLandLord = function()
    ...
    if ownedMotel then 
        table.insert(menuElements, {
            ["label"] = "Sälj ditt motell-rum för (" .. Config.MotelPrice / 10 .. ":-)",
            ["action"] = "sell"
        })
		...
		ESX.UI.Menu.Open("default", GetCurrentResourceName(), "motel_main_dressing_menu", {
			...
			TriggerEvent("skinchanger:getSkin", function(skin)
                ESX.TriggerServerCallback("qalle_motels:getPlayerOutfit", function(clothes)
                    ...
                    ESX.ShowNotification("Du bytte kläder.")
                end, currentOutfit)
			end)
        end, function(menuData, menuHandle)
			menuHandle.close()
