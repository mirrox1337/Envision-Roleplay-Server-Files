...
OpenControlMenu = function()
    local menuElements = {
        {
            ["label"] = "Lägg ut ny möbel.",
            ["action"] = "furnish"
        },
        {
            ["label"] = "Hantera utplacerade möbler.",
            ["action"] = "refurnish"
        }
    }
...
OpenFurnishingMenu = function()
    ...
    if not cachedData["furnishings"][currentMotel] then return ESX.ShowNotification("Du har ingen inredning köpt.") end
    if not cachedData["furnishings"][currentMotel]["ownedFurnishing"] or #cachedData["furnishings"][currentMotel]["ownedFurnishing"] == 0 then return ESX.ShowNotification("Du har ingen inredning köpt.") end
...
ButtonPressed = function(keyLabel)
    ...
    if DoesEntityExist(furHandle) then
        ...
        elseif keyLabel == "ENTER" then
            ...
            ESX.TriggerServerCallback("qalle_motels:saveFurnishing", function(savedFurnishing)
                if savedFurnishing then
                    ...
                    ESX.ShowNotification("Du placerade ut möbeln.")
                else
                    ESX.ShowNotification("Vänligen försök placera ut den igen.")
                end
...
OpenFurnishingComputer = function()
    ...
    if not ownedMotel then return ESX.ShowNotification("Du behöver äga ett motell-rum för att kunna använda datorn.") end
...
ComputerTick = function(currentProp)
    local PurchaseProp = function()
        ...
        ESX.TriggerServerCallback("qalle_motels:purchaseFurnishing", function(purchased)
            DrawBusySpinner("Verifierar...")
            ...
            if purchased then
                ESX.ShowNotification("Köpet lyckades, den finns nu i ditt motell-rum.")
            else
                ESX.ShowNotification("Köpet misslyckades.")
            end
    ...
    Citizen.CreateThread(function()
        while DoesEntityExist(currentProp["handle"]) do
            ...
            DrawScriptText((cachedData["furnishingProp"]["coords"] or Config.MegaMall["object"]["pos"]) - vector3(0.0, 0.0, 0.25), "[~g~G~s~] för att köpa.")
