...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if ( IsControlJustPressed(0,121) and IsPedInAnyVehicle(GetPlayerPed(-1)) and IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) ) then
...
AddEventHandler("esx_pursuit_helper:trackerset", function(veh)
  ...
  if not tracking then
    tracking = true
		showNotification("~w~~o~GPS spårningen började")
		soundconnect = true
    if deployed then
      gpstarget = gps
    ...
    while deployed or tracking do
      ...
      if veh ~= nil then
          if not IsEntityAVehicle(veh) then
              showNotification("~w~~o~GPS Signal förlorad")
							soundconnlost = true
							deployed = false
              tracking = false
          end
...
AddEventHandler("esx_pursuit_helper:trackerremove", function(gps)
    RemoveBlip(gps)
    showNotification("~w~~o~GPS avaktiverad")
		sounddisconnect = true
    tracking = false
end)
