...
Citizen.CreateThread(function ()
  ...
  while true do
    Wait(time)
		if PlayerData.job ~= nil and PlayerData.job.name ~= 'unemployed' and PlayerData.job.name == "ambulance" then
			v = Config.Marker
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, 38) and inMarker then
			ESX.TriggerServerCallback('jsfour-medicalrecord:fetch', function( d )
...
RegisterNUICallback('search', function(data, cb)
	ESX.TriggerServerCallback('jsfour-medicalrecord:search', function( d )
    cb(d)
  end, data)
end)
...
RegisterNUICallback('add', function(data, cb)
	ESX.TriggerServerCallback('jsfour-medicalrecord:add', function( d )
    cb(d)
  end, data)
end)
...
RegisterNUICallback('update', function(data, cb)
	ESX.TriggerServerCallback('jsfour-medicalrecord:update', function( d )
    cb(d)
  end, data)
end)
...
RegisterNUICallback('remove', function(data, cb)
	ESX.TriggerServerCallback('jsfour-medicalrecord:remove', function( d )
    cb(d)
  end, data)
end)
