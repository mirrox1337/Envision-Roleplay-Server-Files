...
UTK = {
    ...
    alarmblip,
    text = { -- Texts
        loudstart = "[~r~E~w~] Starta ~g~Högljutt~w~ bankrån",
        silentstart = "[~r~E~w~] Starta ~g~Tyst~w~ bankrån",
        usecard = "[~r~E~w~] Använd ID Kort",
        usethermal = "[~r~E~w~] Använd Termisk laddning",
        usehack = "[~r~E~w~] Hacka Panelen",
        uselockpick = "[~r~E~w~] Använd Lockpick",
        usesearch = "[~r~E~w~] Sök igenom",
        lootcash= "[~r~E~w~] Länsa pengavagnen",
        lootgold = "[~r~E~w~] Länsa guldvagnen",
        lootdia = "[~r~E~w~] Länsa diamantvagnen",
        card = "Använder ID Kort",
        thermal = "Planterar Termisk laddning",
        burning = "Smälter låset",
        lockpick = "Pickar låset",
        using = "Panel Progress",
        used = "Process complete.",
        stage = "Klart.",
        search = "Söker",
        hacking = "Hackar",
        melted = "Dörrlåset smälte.",
        hacked = "Hack completed.",
        unlocked = "Dörr olåst.",
        nothing = "Hitta ingenting.",
        found = "Du hitta ID-kortet.",
        time = "Tid kvar: "
    }
}
UTK.searchinfo = {
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            ...
                                            elseif not output then
                                                exports['mythic_notify']:SendAlert("error", "Du har inte rätt laptop.")
                                            end
                                        end, "laptop_h")
                                    end
                                end
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ...
                                        if output then
                                            ...
                                            self:IdCard()
                                        elseif not output then
                                            exports["mythic_notify"]:SendAlert("error", "Du har inte ID-kortet.")
                                        end
                                        end, "id_card")
                                    end
                                end
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                ...
                                                self:Plant()
                                            elseif not output then
                                                exports['mythic_notify']:SendAlert("error", "Du har ingen Termisk laddning.")
                                            end
                                        end, "thermal_charge")
                                    end
                                end
                                        ...
                                        if dst <= 1 and IsControlJustReleased(0, 38) then
                                            ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                                if output then
                                                    ...
                                                    self:Lockpick()
                                                elseif not output then
                                                    exports['mythic_notify']:SendAlert("error", "Du har inga lockpicks.")
                                                end
                                            end, "lockpick")
                                        end
                                    end
...
function UTK:Hack()
    ...
    Citizen.Wait(2000)
    Brute()
    exports["mythic_notify"]:SendAlert("success", "Öppna datorn och navegera till HackConnect.exe")
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                if program == 82 and not Hacking then
                    ...
                    PopScaleformMovieFunctionVoid()
                    Hacking = true
                    exports["mythic_notify"]:SendAlert("success", "Hitta IP-adressen...")
                elseif program == 83 and not Hacking and Ipfinished then
                    ...
                    Hacking = true
                    exports["mythic_notify"]:SendAlert("success", "Hitta lösenordet...")
                ...
                elseif Hacking and program == 84 then
                    ...
                    Hacking = false
                    Ipfinished = true
                    exports["mythic_notify"]:SendAlert("success", "Kör BruteForce.exe")
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        exports['mythic_notify']:SendAlert("error", "Du har 90 sekunder tills farlig GAS släpps in.")
        UTK.starttimer = true
        UTK.vaulttime = 90
        Citizen.CreateThread(function()
            while true do
        ...
        ESX.TriggerServerCallback("utk_oh:gettotalcash", function(result)
            text = "SEK"..ESX.Math.GroupDigits(result)
            exports["mythic_notify"]:SendAlert("success", "Du fick med dig "..text)
        end)
    else
        exports['mythic_notify']:SendAlert("error", "Du har 90 seconds tills farlig GAS släpps in i valvet.")
        UTK.starttimer = true
        UTK.vaulttime = 90
        Citizen.CreateThread(function()
            while true do
		...
		ESX.TriggerServerCallback("utk_oh:gettotalcash", function(result)
            text = "SEK"..ESX.Math.GroupDigits(result)
            exports["mythic_notify"]:SendAlert("success", "Du stal "..text)
        end)
    end
end)
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        exports["mythic_notify"]:SendAlert("error", "Valvets dörr stängs automatiskt om 60 sekunder!")
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    ...
    if player.job.name == "police" then
        if toggle == 1 then
            exports["mythic_notify"]:SendAlert("infrom", "Pacific Standart Bank alarm har triggats!!!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
