...
AddEventHandler('esx_heligarage:hasEnteredMarker', function(zone)
       ...
       if PlayerData.job.name == 'police' or PlayerData.job.name == 'ambulance' then
        if zone == 'PoliceGet' then
            CurrentActionMsg  = 'Tryck ~INPUT_CONTEXT~ för att få ~o~helikopter'
        ...
        if zone == 'PoliceRem' then
            CurrentActionMsg = 'Press ~INPUT_CONTEXT~ to remove ~o~helikopter'
...
Citizen.CreateThread(function()
    while true do
        ...
        if CurrentAction == ACTION_GET_POLICEHELI then
            ESX.ShowNotification('~o~Helikoptern~w~ är påväg..')
            Citizen.Wait(6000)
        ...
        if CurrentAction == ACTION_REMOVE_POLICEHELI then
            ESX.ShowNotification('Tar bort  ~o~helikoptern')
            Citizen.Wait(4500)
