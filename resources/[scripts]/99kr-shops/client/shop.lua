    ...
    while true do
        ...
        if payAmount > 0 then
            for shop = 1, #Config.Locations do
                ...
                if dist <= 20.0 then
                    if dist >= 12.0 then
                        pNotify("Glöm det tattare, här betalar vi!", "error", 2500)
                        payAmount = 0
                        Basket = {}
                    end
                end
...
OpenAction = function(action, shelf, text)
    if action["value"] == "checkout" then
        if payAmount > 0 and #Basket then
            ...
            pNotify("Du har ingenting i korgen!", 'error', 1500)
        end
    else
...
CashRegister = function(titel)
        local elements = {
            {label = '<span style="color:lightgreen; border-bottom: 1px solid lightgreen;">Bekräfta</span>', value = "yes"},
            {label = 'Belopp att betala: <span style="color:green">' .. payAmount ..'</span>'},
        }

        for i=1, #Basket do
            local item = Basket[i]
            table.insert(elements, {
                label = '<span style="color:red">*</span> ' .. item["label"] .. ': ' .. item["amount"] .. ' st',
                value = item["value"],
            })
        end
                ...
                if data.current.value == "yes" then
                    ...
                    ESX.UI.Menu.Open(
                        'default', GetCurrentResourceName(), 'penis2',
                        {
                            ...
                            align    = 'center',
                            elements = {
                                {label = "Betala kontant", value = "cash"},
                                {label = "Betala med kreditkort", value = "bank"},
                            },
                        },
                        function(data2, menu2)
                            ESX.TriggerServerCallback('99kr-shops:CheckMoney', function(hasMoney)
                                if hasMoney then
                                    ...
                                    pNotify("Du har inte tillräckligt med pengar!", 'error', 1500)
                                end
...
ShelfMenu = function(titel, shelf)
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            pNotify("La " .. data.current.value .. " st " .. data.current["realLabel"] .. " i korgen", 'alert', 1500)           
        end,
    function(data, menu)
        menu.close()
    end)
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                if data.current.value == 'empty' then
                    ...
                    payAmount = 0
                    menu.close()
                    pNotify("Tog bort allt ur korgen.", "error", 2500)
                end
                if data.current.value == "item_menu" then
                    ...
                    ESX.UI.Menu.Open(
                        'default', GetCurrentResourceName(), 'basket_detailedmenu',
                        {
                            ...
                            align    = 'center',
                            elements = {
                                {label = shopItem["label"] .. " - SEK" .. shopItem["price"] * shopItem["amount"]},
