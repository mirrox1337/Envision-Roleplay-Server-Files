...
AddEventHandler("bankrobberies:eventHandler", function(event, eventData)
	...
	elseif event == "alarm_police" then
		if ESX.PlayerData["job"] and ESX.PlayerData["job"]["name"] == "police" then
			...
			ESX.ShowNotification("BANKRÅN - " .. eventData .. ". SLÄPP ALLT OCH ÅK!!!")
		...
		for bank, values in pairs(Config.Banks) do
			...
			if dstCheck <= 5.0 then
				...
				if dstCheck <= 1.0 then
					local usable, displayText = not cachedData["banks"][bank], cachedData["hacking"] and "Hackar..." or cachedData["banks"][bank] and "Valvet är öppet." or "Använd ~INPUT_CONTEXT~ för att hacka valvet"
