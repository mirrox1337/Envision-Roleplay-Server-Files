...
function updateTokovoipInfo(msg) {
	...
	switch (voipStatus) {
		...
		case INCORRECT_VERSION:
			msg = 'Felaktig Plugin Version';
			screenMessage = 'Använder felaktig Plugin Version';
			color = 'red';
			break;
		case OK:
			color = '#f2f2f7';
	...
	if (msg) {
		document.getElementById('tokovoipInfo').innerHTML = `<font color="${color}">[RÖST] ${msg}</font>`;
	}
...
function updateConfig(payload) {
	voip = payload;
	document.getElementById('TSServer').innerHTML = `TeamSpeak Server IP: <font color="#0a84ff">${voip.plugin_data.TSServer}</font>`;
	document.getElementById('TSChannel').innerHTML = `TeamSpeak Kanal: <font color="#0a84ff">${(voip.plugin_data.TSChannelWait) ? voip.plugin_data.TSChannelWait : voip.plugin_data.TSChannel}</font>`;
