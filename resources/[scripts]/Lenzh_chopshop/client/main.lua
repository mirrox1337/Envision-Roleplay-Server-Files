...
function ChopVehicle()
    ...
    if seats ~= 0 then
        TriggerEvent('chat:addMessage', { args = { '[^1Chopshop^0]: Du kan inte skrota med en passagerare' } })
    elseif
        ...
        ESX.TriggerServerCallback('Lenzh_chopshop:anycops', function(anycops)
            if anycops >= Config.CopsRequired then
                ...
                if not HasAlreadyEnteredMarker then
                    ...
                    exports.pNotify:SendNotification({text = "Du lämnade zonen, inga bildelar tilldelades dig", type = "error", timeout = 1000, layout = "centerRight", queue = "right", killer = true, animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        ...
        exports.pNotify:SendNotification({
            text = "Kom tillbaka om " ..math.floor(timerNewChop / 60000).. "minuter ",
            type = "error",
            timeout = 1000,
            layout = "centerRight",
            queue = "right",
...
AddEventHandler('lenzh_chopshop:hasExitedMarker', function(zone)
    ...
    if zone == 'Chopshop' then
        ...
        if ChoppingInProgress == true then
            exports.pNotify:SendNotification({text = "Du lämnar zonen, gå tillbaka", type = "error", timeout = 1000, layout = "centerRight", queue = "right", killer = true, animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        end
    end
    ChoppingInProgress        = false
