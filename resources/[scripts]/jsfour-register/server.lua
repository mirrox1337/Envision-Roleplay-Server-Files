...
ESX.RegisterServerCallback('jsfour-register:register', function(source, cb, data)
  ...
  MySQL.Async.execute("UPDATE `users` SET `firstname` = @firstname, `lastname` = @lastname, `dateofbirth` = @dateofbirth, `sex` = @sex, `height` = @height, `blodgrupp` = @blodgrupp, `lastdigits` = @lastdigits WHERE identifier = @identifier",
  {
    ...
    ['@sex']          = data.sex,
    ['@height']       = data.height,
    ['@blodgrupp']    = data.blodgrupp,
    ['@lastdigits']   = lastdigits
   },
  function( result )
    MySQL.Async.execute('INSERT INTO characters (identifier, firstname, lastname, dateofbirth, sex, height, blodgrupp, lastdigits) VALUES (@identifier, @firstname, @lastname, @dateofbirth, @sex, @height, @blodgrupp, @lastdigits)',
  {
    ['@identifier']   = identifier,
    ['@firstname']    = data.firstname,
    ['@lastname']     = data.lastname,
...
function getIdentity(source, callback)
  local identifier = GetPlayerIdentifiers(source)[1]
  MySQL.Async.fetchAll("SELECT identifier, firstname, lastname, dateofbirth, sex, height, blodgrupp, lastdigits FROM `users` WHERE `identifier` = @identifier",
  {
    ['@identifier'] = identifier
  },
  function(result)
