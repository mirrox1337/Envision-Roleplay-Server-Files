    ...
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
        ...
        for key, vehicleData in ipairs(fetchedVehicles) do
            ...
            table.insert(menuElements, {
                ["label"] = "Använd " .. GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " with plate - " .. vehicleData["plate"],
                ["vehicle"] = vehicleData
            })
        end
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "Du har inga fordon parkerade här."
            })
...
OpenVehicleMenu = function()
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
            ...
            for _, vehicle in ipairs(gameVehicles) do
                if DoesEntityExist(vehicle) then
                    ...
                    if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                        table.insert(menuElements, {
                            ["label"] = GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " med reg - " .. vehicleData["plate"] .. " - " .. dstCheck .. " meter.",
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "Du har inga fordon på gatan."
            })
        end
...
ChooseVehicleAction = function(vehicleEntity, callback)
    ...
    local menuElements = {
        {
            ["label"] = (GetVehicleDoorLockStatus(vehicleEntity) == 1 and "Lås" or "Lås upp") .. " Fordonet.",
            ["action"] = "change_lock_state"
        },
        {
            ["label"] = "sätt/stäng " .. (GetIsVehicleEngineRunning(vehicleEntity) and "av" or "på") .. " fordonet.",
            ["action"] = "change_engine_state"
        },
        {
            ["label"] = "sätt/stäng " .. (DoesBlipExist(cachedData["blips"][vehicleEntity]) and "av" or "på") .. " gps .",
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "second_vehicle_menu", {
        ["title"] = "Välj vad du vill göra med - " .. GetVehicleNumberPlateText(vehicleEntity),
        ["align"] = Config.AlignMenu,
        ["elements"] = menuElements
    }, function(menuData, menuHandle)
...
VehicleAction = function(vehicleEntity, action)
    ...
    if action == "change_lock_state" then
        if dstCheck >= Config.RangeCheck then return ESX.ShowNotification("För långt avstånd i från fordonet.") end
        ...
        ESX.ShowNotification(GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicleEntity))) .. " Med reg - " .. GetVehicleNumberPlateText(vehicleEntity) .. " Är nu " .. (vehicleLockState == 1 and "LÅST" or "UPPLÅST"))
    elseif action == "change_door_state" then
        if dstCheck >= Config.RangeCheck then return ESX.ShowNotification("För långt avstånd ifrån fordonet.") end
...
SpawnLocalVehicle = function(vehicleProps)
	...
	if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		ESX.ShowNotification("Flytta på fordonet som är i vägen.")
...
SpawnVehicle = function(vehicleProps)
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
				ESX.ShowNotification("Du kan inte ta ut 2 st av samma fordon.")
	...
	ESX.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
        ...
        ESX.ShowNotification("Du tog ut ditt fordon ur garaget.")
	...
	if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("garage:validateVehicle", function(valid)
			if valid then
				...
				ESX.ShowNotification("Du parkerade ditt fordon.")
			else
				ESX.ShowNotification("Äger du verkligen detta fordon?")
			end
